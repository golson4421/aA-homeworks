...
def sluggish_fish(array)
    current_longest = nil 
    current_longest_length = 0
    array.each do |fish|
      if fish.length >= current_longest_length
           current_longest = fish 
           current_longest_length = fish.length
      end
    end
...
#O(n) the n inside of the (), refers to the amount of times it will take to find it???
...
#O(1) refers to the constant that is ussed
